  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setLoading(false);
        toast.error(
          response.data?.error || "Failed to fetch transferred production details"
        );
        return;
      }
      ...
      console.error("Fetch transferred production details error:", error);
      setLoading(false);
      toast.error("Failed to load transferred production details");
    }
  };
    ...
    return [
      { id: "key", label: "S/N" },
      {
        ...
        format: (value, row) => {
          ...
          const totalItems = row.ShiftedItem.length;
          const totalQuantity = row.ShiftedItem.reduce(
            (sum, item) => sum + Number(item.Shifted_Balance),
            0
          );
    ...
    <>
      ...
      <div className="w-full h-12">
        <div className="w-full my-2 flex justify-between">
          <h4>Pending Transfers List</h4>
          <div className="flex gap-2">
            <div
              onClick={() => navigate("/projects/oxygen/pending-transfers/receive")}
            ...
            >
              ...
              <p className="py-2">Receive Productions</p>
            </div>
          </div>
        </div>
      </div>
