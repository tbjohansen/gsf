...
const EditWing = ({ wing, loadData }) => {
  ...
  const [name, setName] = useState(wing?.Wing_Name);
  const [status, setStatus] = useState({
    id: wing?.Wing_Status,
    label: capitalize(wing?.Wing_Status),
  });
  const [gender, setGender] = useState({
    id: wing?.Wing_Gender,
    label: capitalize(wing?.Wing_Gender),
  });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        Wing_Status: status?.id,
        Wing_Gender: gender?.id,
        Block_ID: blockID,
      ...
      // Success
      setLoading(false);
      toast.success("Wing is updated successfully");
    ...
    <div>
      ...
      >
        <Box sx={style} className="rounded-md">
          <div>
            <h3 className="text-center text-xl py-4">Edit Block Wing</h3>
            <div>
