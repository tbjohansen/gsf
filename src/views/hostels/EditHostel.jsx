...
const EditHostel = ({ hostel, loadData }) => {
  ...
  const [name, setName] = useState(hostel?.Hostel_Name);
  const [status, setStatus] = useState({
    id: hostel?.Hostel_Status,
    label: capitalize(hostel?.Hostel_Status),
  });
  ...
  const createHostel = async (e) => {
    ...
    try {
      ...
      const data = {
        Hostel_Name: name.trim(),
        Hostel_Status: status?.id,
      ...
      if (!response.ok) {
        ...
        if (response.problem === "NETWORK_ERROR") {
          ...
          toast.error(response.data?.error || "Failed to update hostel");
        }
        return;
      }
      ...
      if (response.data?.error || response.data?.code >= 400) {
        setLoading(false);
        const errorMessage = response.data.error || "Failed to update hostel";
    ...
    <div>
      ...
      >
        <Box sx={style} className="rounded-md">
          <div>
            <h3 className="text-center text-xl py-4">Edit Hostel Details</h3>
            <div>
