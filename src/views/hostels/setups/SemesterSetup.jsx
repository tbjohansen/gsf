...
export default function SemesterSetup() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get(`/settings/semester-control`);
      ...
      setSemesters(Array.isArray(newData) ? newData : []);
  ...
  const columns = useMemo(
    () => [
      ...
      {
        id: "Semester_Opening_Date",
        label: "Opening Window",
        format: (value) => <>{formatDateForDb(value)}</>,
      },
      {
        id: "semesterOne",
        label: "Semester One",
        format: (value, row) => <div><p>Start: {formatDateForDb(row?.Semester_One_Start_Date)}</p><p>End:  {formatDateForDb(row?.Semester_One_End_Date)}</p></div>,
      },
      {
        id: "semesterTwo",
        label: "Semester Two",
        format: (value, row) => <div><p>Start: {formatDateForDb(row?.Semester_Two_Start_Date)}</p><p>End:  {formatDateForDb(row?.Semester_Two_End_Date)}</p></div>,
      },
      {
        id: "Semester_Status",
        label: "Status",
      ...
      },
      {
        ...
        format: (value, row) => (
          <div className="flex gap-4 justify-center">
            <EditSemester semester={row} loadData={loadData} />
          </div>
        ),
      },
    ],
