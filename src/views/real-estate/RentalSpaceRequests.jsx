...
export default function RentalSpaceRequests() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      let url = `/customer/customer-request?&Request_Type=business_land&limit=${rowsPerPage}&page=${page}`;
  ...
  const handleDownloadPDF = () => {
    ...
    doc.save("GSF-Rental-Space-Payments.pdf");
  };
  ...
  const handleDownloadExcel = () => {
    const tableData = payments?.map((row, index) => ({
      ...
      Unit: row?.room?.hostel?.Hostel_Name,
    ...
    summaryRows.push(
      ...
      ["Total Business Units", 0],
      ["Occupied Business Units", 0],
      ["Available Business Units", 0],
    );
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "GSF Rental Space Payments",
    );
    XLSX.writeFile(workbook, `GSF-Rental-Space-Payments-${Date.now()}.xlsx`);
  };
