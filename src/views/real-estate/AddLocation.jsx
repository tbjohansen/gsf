...
const AddLocation = ({ loadData }) => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.post("/settings/unit-location", data);
      ...
      if (!response.ok) {
        ...
        if (response.problem === "NETWORK_ERROR") {
          ...
          toast.error(response.data?.error || "Failed to create location");
        }
        return;
      }
  ...
  return (
    <div>
      ...
      >
        <MdAdd className="my-3" /> <p className="py-2">Create New Location</p>
      </div>
      ...
      >
        <Box sx={style} className="rounded-md">
          <div>
            <h3 className="text-center text-xl py-4">Add New Location</h3>
            <div>
