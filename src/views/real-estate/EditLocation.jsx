...
const EditLocation = ({ location, loadData }) => {
  ...
  const [description, setDescription] = useState(location?.Unit_Location);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (response.data?.error || response.data?.code >= 400) {
        setLoading(false);
        const errorMessage = response.data.error || "Failed to update location";
    ...
    <div>
      ...
      >
        <Box sx={style} className="rounded-md">
          <div>
            <h3 className="text-center text-xl py-4">Edit Location Details</h3>
            <div>
              ...
              <div className="w-full py-2 mt-5 flex justify-center">
                ...
                >
                  {loading ? "Updating..." : "Edit Location"}
                </button>
              </div>
            </div>
          </div>
